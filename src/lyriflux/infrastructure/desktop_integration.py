...
LEGACY_APP_ID = "io.github.myonctl.LyricFlow"
...
class DesktopIntegrationStatus:
    ...
    icon_installed: bool
    legacy_desktop_installed: bool
    legacy_icon_installed: bool
...
def integration_status(data_home: Path) -> DesktopIntegrationStatus:
    ...
    legacy_desktop, legacy_icon = _legacy_integration_paths(data_home)
    return DesktopIntegrationStatus(
        desktop_file,
        icon_file,
        desktop_file.is_file(),
        icon_file.is_file(),
        legacy_desktop.is_file(),
        legacy_icon.is_file(),
    )


def _legacy_integration_paths(data_home: Path) -> tuple[Path, Path]:
    return (
        data_home / "applications" / f"{LEGACY_APP_ID}.desktop",
        data_home / "icons" / "hicolor" / "scalable" / "apps" / f"{LEGACY_APP_ID}.svg",
    )


def _validate_known_legacy_integration(data_home: Path) -> bool:
    desktop_file, _icon_file = _legacy_integration_paths(data_home)
    ...
    try:
        ...
        raise DesktopIntegrationError(
            "the legacy LyricFlow launcher could not be inspected; it was retained"
        ) from error
    known_launcher = (
        "[Desktop Entry]" in content
        and "Name=LyricFlow" in content
        and " desktop" in content
        and "lyricflow" in content.casefold()
    )
    if not known_launcher:
        raise DesktopIntegrationError(
            "the legacy launcher does not match LyricFlow's known project-owned "
            "metadata; it was retained"
        )
    return True


def _remove_known_legacy_integration(data_home: Path) -> None:
    desktop_file, icon_file = _legacy_integration_paths(data_home)
    if not _validate_known_legacy_integration(data_home):
        return
    try:
        desktop_file.unlink()
        ...
        raise DesktopIntegrationError(
            "the known legacy LyricFlow launcher could not be removed"
        ) from error
...
def _resource_bytes(name: str) -> bytes:
    return files("lyriflux").joinpath("resources", name).read_bytes()
...
def install_desktop_integration(
    ...
    if not resolved_executable.is_file() or not os.access(resolved_executable, os.X_OK):
        raise DesktopIntegrationError(
            "the LyriFlux executable could not be resolved to an executable file"
        )
    status = integration_status(data_home)
    _validate_known_legacy_integration(data_home)
    template = _resource_bytes(f"{APP_ID}.desktop.in").decode("utf-8")
    rendered = (
        template.replace(
            "@LYRIFLUX_EXECUTABLE@", _desktop_exec_argument(str(resolved_executable))
        )
        .replace(
            "@LYRIFLUX_ICON@",
    ...
    try:
        ...
        _remove_known_legacy_integration(data_home)
    except OSError as error:
...
def remove_desktop_integration(data_home: Path) -> DesktopIntegrationStatus:
    """Remove only the exact launcher and icon owned by LyriFlux."""
...
def current_executable(command: str) -> Path:
    ...
    if located is None:
        ...
        if candidate.is_absolute():
            return candidate
        raise DesktopIntegrationError("could not locate the lyriflux console script")
    return Path(located)
