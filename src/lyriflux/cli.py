...
from lyriflux import __version__
from lyriflux.application.diagnostics import build_doctor_report
from lyriflux.application.player_diagnostics import (
    render_player_event,
...
from lyriflux.application.selection_diagnostics import render_player_selection
...
def _create_audio_latency_probe() -> AudioLatencyProbePort:
    ...
    from lyriflux.infrastructure.audio.pipewire import PipeWireLatencyProbe
...
def _run_watch(runtime: MprisRuntimePort) -> int:
    def show_event(event: object) -> None:
        from lyriflux.domain.models import PlayerEvent
...
def _run_players(
    ...
    if arguments.players_command == "select":
        ...
        try:
            ...
            print(f"Unable to use LyriFlux storage: {error}", file=sys.stderr)
            return 1
...
def _run_storage(
    ...
    if database_path is None:
        try:
            ...
            print(
                f"Unable to migrate legacy LyricFlow storage: {error}",
                file=sys.stderr,
            )
            return 1
    ...
    try:
        if arguments.storage_command == "backup":
            ...
            if status.error is not None or status.migration_status != "current":
                print(
                    "Unable to back up LyriFlux storage: storage must be healthy "
                    "and current; run `lyriflux storage status`.",
                    file=sys.stderr,
                )
                return 1
            storage = open_storage_readonly(path)
            backup = backup_database(storage.database, arguments.destination)
            print("Created verified LyriFlux storage backup.")
        ...
        if arguments.storage_command == "migrate":
            print(
                f"LyriFlux storage is current at schema version "
...
def _run_desktop_integration(
    ...
    print(
        "legacy LyricFlow launcher present: "
        f"{'yes' if status.legacy_desktop_installed else 'no'}"
    )
    print(
        "legacy LyricFlow icon present: "
        f"{'yes' if status.legacy_icon_installed else 'no'}"
    )
    print("application data modified: no")
    return int(
        arguments.desktop_integration_command == "status"
        and (not status.installed or status.legacy_desktop_installed)
    )
...
def _run_diagnostics(arguments: argparse.Namespace, database_path: Path | None) -> int:
    ...
    from lyriflux.infrastructure.release_diagnostics import (
...
def _run_library(
    ...
    from lyriflux.infrastructure.lyrics.library_download import LibraryLyricsDownloader
    ...
    try:
        ...
        if arguments.library_command == "review":
            ...
            print(f"LyriFlux music-library review queue ({len(items)})")
            for item in items:
...
def _run_lyrics(
    ...
    from lyriflux.application.lyrics_diagnostics import render_lyrics_resolution
    ...
    from lyriflux.infrastructure.lyrics.local_sidecar import (
    ...
    if arguments.lyrics_command == "current":
        ...
        if result.document is None:
            ...
            return 1
        from lyriflux.application.representation_diagnostics import (
            render_representations,
        )
        from lyriflux.application.representations import RepresentationService
        from lyriflux.domain.lyrics import RepresentationKind
...
def _resolve_sync_document(
    ...
    *,
    ...
    from lyriflux.application.resolve_lyrics import LyricsResolver
    from lyriflux.domain.lyrics import LyricDocumentKind
    from lyriflux.infrastructure.lyrics.embedded import EmbeddedLyricsProvider
...
def _run_sync_audio(
    ...
    from lyriflux.application.sync_diagnostics import render_audio_latency_probe
    from lyriflux.domain.synchronization import OutputDeviceCalibration
    from lyriflux.infrastructure.storage.bootstrap import open_storage
    from lyriflux.infrastructure.storage.errors import StorageError
...
def _run_sync_probe(
    ...
    from lyriflux.application.sync_diagnostics import render_probe_report
...
def _run_sync(
    ...
    from lyriflux.application.playback_clock import PlaybackClock
    from lyriflux.application.representations import RepresentationService
    from lyriflux.application.sync_diagnostics import (
