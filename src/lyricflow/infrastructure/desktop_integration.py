...
def install_desktop_integration(
    ...
    if not resolved_executable.is_file() or not os.access(resolved_executable, os.X_OK):
        raise DesktopIntegrationError(
            "the LyricFlow executable could not be resolved to an executable file"
        )
...
def remove_desktop_integration(data_home: Path) -> DesktopIntegrationStatus:
    """Remove only the exact launcher and icon owned by LyricFlow."""
...
def current_executable(command: str) -> Path:
    ...
    if located is None:
        ...
        if candidate.is_absolute():
            return candidate
        raise DesktopIntegrationError("could not locate the lyricflow console script")
    return Path(located)
