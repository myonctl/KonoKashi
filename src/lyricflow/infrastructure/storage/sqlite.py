...
from lyricflow.infrastructure.storage.migrations import MIGRATIONS, Migration
    ...
    def initialize(self, migrations: Sequence[Migration] = MIGRATIONS) -> int:
        ...
        if applied and applied[-1][0] > len(ordered):
            raise UnsupportedSchemaError(
                "database schema is newer than this LyricFlow build; upgrade the app",
                path=self.path,
            )
        for version, name, checksum, _applied_at in applied:
            ...
            if (
                ...
                raise UnsupportedSchemaError(
                    "database migration history differs from this LyricFlow build",
                    path=self.path,
                )
