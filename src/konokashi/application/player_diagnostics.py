...
def _property_value(snapshot: PlayerSnapshot | None, name: str) -> object | None:
    if snapshot is None:
        return None
    direct_values: dict[str, object | None] = {
        "Identity": snapshot.identity,
        "DesktopEntry": snapshot.desktop_entry,
        "PlaybackStatus": snapshot.playback_status,
        "Position": snapshot.position_us,
        "LoopStatus": snapshot.loop_status,
        "Rate": snapshot.rate,
        "Shuffle": snapshot.shuffle,
        "Volume": snapshot.volume,
        "MinimumRate": snapshot.minimum_rate,
        "MaximumRate": snapshot.maximum_rate,
        "SupportedUriSchemes": snapshot.supported_uri_schemes,
        "SupportedMimeTypes": snapshot.supported_mime_types,
    }
    capability_values: dict[str, object | None] = {
        "CanQuit": snapshot.capabilities.can_quit,
        "CanRaise": snapshot.capabilities.can_raise,
        "HasTrackList": snapshot.capabilities.has_track_list,
        "CanGoNext": snapshot.capabilities.can_go_next,
        "CanGoPrevious": snapshot.capabilities.can_go_previous,
        "CanPlay": snapshot.capabilities.can_play,
        "CanPause": snapshot.capabilities.can_pause,
        "CanSeek": snapshot.capabilities.can_seek,
        "CanControl": snapshot.capabilities.can_control,
    }
    return direct_values.get(name, capability_values.get(name))


def _properties_event_details(event: PlayerEvent) -> str:
    details = [
        f"{name}={_value(_property_value(event.snapshot, name))}"
        for name in event.changed_properties
    ]
    details.extend(f"{name}=<invalidated>" for name in event.invalidated_properties)
    return "; ".join(details) if details else "no recognized property values"
...
def render_player_event(event: PlayerEvent) -> str:
    ...
    elif event.kind is PlayerEventKind.PROPERTIES_CHANGED:
        detail = _properties_event_details(event)
